...
| 要素 | 0.14.1 | v0 現状 | 必要対応 |
|------|--------|---------|----------|
| **wrap** | `wrap install`（PATH に atrakta を置く）、`wrap run --interface <id> --real <path> -- [args]`（実ツールを atrakta 経由で起動）。 | `install／uninstall／run` を実装済み。binding 駆動の script 生成と audit append まで導入済み。 | run 側の実ツール連携と trigger 情報の拡張、binding の追加が残る。 |
...
| **ide-autostart** | `.vscode/tasks.json` 等で「ワークスペースを開いたら atrakta start を実行」等。 | `ide-autostart` を実装済み。`.vscode/tasks.json` と `.cursor/autostart.json` を生成できる。 | JetBrains / Neovim 向け等の追加サーフェスは後続。 |
...
| 要素 | 0.14.1 | v0 現状 | 必要対応 |
|------|--------|---------|----------|
| **projection render** | 契約・正規状態から、インターフェース別に `agents_md` 等を**ディスクに書き出す**。 | 実装済み。`AGENTS.md` に加えて `ide_rules` / `repo_docs` / `skill_bundle` の target まで deterministic 出力が可能。 | parity 連携、マージ戦略、差分表示の強化を行う。 |
