...
[English](../../en/03_operations/01_setup.md) | [日本語](./01_導入手順.md)
...
- `--no-overwrite` 指定時、上書き衝突は `.atrakta/proposals/*.patch` に提案として出力される
- merge 可能な managed 範囲のみ apply される

## 4. 導入後確認（greenfield/brownfield 共通）
...
- `doctor` が BLOCKED でない
- `doctor --parity` が BLOCKED でない
- `doctor --integration` が BLOCKED でない
- `projection status --json` で必要 projection の状態を確認できる
