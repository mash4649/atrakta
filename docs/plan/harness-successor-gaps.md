# v0 as Harness Successor: Gap Analysis

v0 が Atrakta の後継として**ハーネス**（AI ツールとプロジェクト状態の間に立ち、セッションの再開・監査・安全を保つランタイム）の役割を担うために、現状不足している要素を整理する。
この文書はギャップ分析であり、現行の実装・契約は [Implementation Status](implementation-status.md) と
[Run Contract](../architecture/run-contract.md) を優先して参照する。
...
|------|--------|---------|----------|
| **init** | wrap install → hook install → ide-autostart install → start。初回セットアップの単一エントリポイント。 | 最小版は実装済み。`start` 委譲 + `init.*` イベント発火は導入済みで、wrap/hook/ide-autostart の surface コマンドも実装済み。 | `init` と tool binding の最終接続、および start との統合を詰める。 |
| **start** | 契約ロード → Fast Gate 判定 → イベントチェーン検証 → インターフェース解決 → detect → plan → apply → gate → state/progress/events 更新。 | 実装済み。`run` と共通のエントリ解決、contract ロード/検証、preflight 監査検証、Fast Path、runtime state/handoff 更新に加え、P1-H-001 で diagnostics / output-envelope hardening を完了。 | 残課題は binding 駆動のインターフェース解決拡張、wrap/hook 連携、`init` との最終統合。 |
...
**結論**: 入口 3 点のうち **start / resume は実装済み**。`init` も wrap/hook/ide-autostart まで実体化したが、`start` との最終統合や binding / parity の詰めは残っている。
...
| 要素 | 0.14.1 | v0 現状 | 必要対応 |
|------|--------|---------|----------|
| **wrap** | `wrap install`（PATH に atrakta を置く）、`wrap run --interface <id> --real <path> -- [args]`（実ツールを atrakta 経由で起動）。 | `install / uninstall / run` を実装済み。binding-driven script generation と audit append まで導入済み。 | run 側の実ツール連携と trigger 情報の拡張、binding の追加が残る。 |
| **hook** | `hook install|uninstall|status|repair`。IDE 等のサーフェスにフックを仕込み、保存時や起動時に atrakta を噛ませる。 | `install / uninstall / status / repair` を実装済み。Cursor / VS Code 系の hook 管理が可能。 | surface 別の細分化と parity 連携の強化が残る。 |
| **ide-autostart** | `.vscode/tasks.json` 等で「ワークスペース開いたら atrakta start を実行」等。 | `ide-autostart` を実装済み。`.vscode/tasks.json` と `.cursor/autostart.json` を生成できる。 | JetBrains / Neovim 向け等の追加サーフェスは後続。 |

**結論**: ハーネスは「ユーザーが使う AI ツールの前後に atrakta が入る」基盤を持ち始めた。次は **binding / parity / surface 追加** と、`init` との最終接続を詰める段階。
...
| 要素 | 0.14.1 | v0 現状 | 必要対応 |
|------|--------|---------|----------|
| **projection render** | 契約・canonical 状態から、インターフェース別に `agents_md` 等を**ディスクに書き出す**。 | 実装済み。`AGENTS.md` に加えて `ide_rules` / `repo_docs` / `skill_bundle` の target まで deterministic 出力が可能。 | parity 連携、マージ戦略、差分表示の強化を行う。 |
...
| **task-graph.json** | 保留・完了タスクの DAG。 | `.atrakta/task-graph.json` の永続化に加え、resume replay でノード寿命を保持するよう hardening 済み。 | 0.14.1 との意味論差分を必要に応じて明文化する。 |
| **Fast Path スナップショット** | `.atrakta/runtime/meta.v2.json` の `start_fast_v2`。contract_hash / workspace_stamp / interface set / feature_id / config key が一致すれば strict パスをスキップ。 | `.atrakta/runtime/start-fast.v1.json` で no-change short-circuit を実装済み。`apply_requested` をキーに含め、互換性を維持したまま policy を拡張済み。 | 0.14.1 との差分（メタデータ・互換条件）を整理する。 |
...
**結論**: runtime state の最小セット（state/progress/task-graph/Fast Path/auto-state）は実装済み。残りは **互換方針と意味論の明文化**。
...
**結論**: ハーネスの中核（init/start/resume、wrap/hook、projection、state/events）よりは優先度は下がるが、0.14.1 と同程度の「外部資産の取り込み」を v0 でも扱うなら、**import + capability + recipe/memory/exploration** の順で足していく形が自然。
...
1. **必須（ハーネス運用完成のため）**
   ...
   - **init** の残り統合（wrap/hook/ide-autostart の実体化）
   - **run-events** のイベント種別拡張と検証範囲の整備

2. **重要（運用・一貫性）**
   - **gc**、**migrate check** のポリシー拡張
