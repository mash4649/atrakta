...
func AcceptOnboarding(projectRoot string, bundle onboarding.ProposalBundle) (AcceptResult, error) {
	...
	if _, err := audit.AppendEvent(filepath.Join(storeRoot, "audit"), auditLevel, "accept_onboarding", map[string]any{
		"inferred_mode":  bundle.InferredMode,
		"conflicts":      bundle.Conflicts,
	...
	if err := audit.VerifyIntegrity(filepath.Join(storeRoot, "audit"), auditLevel); err != nil {
		return AcceptResult{}, fmt.Errorf("accept audit verify: %w", err)
	}
