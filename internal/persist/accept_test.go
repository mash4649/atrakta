...
func TestAcceptOnboarding(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, ".atrakta/generated/acceptance-spec.generated.json")); err != nil {
		t.Fatalf("missing acceptance spec: %v", err)
	}
	if _, err := os.Stat(filepath.Join(root, ".atrakta/generated/acceptance-rubric.generated.json")); err != nil {
		t.Fatalf("missing acceptance rubric: %v", err)
	}
	if _, err := os.Stat(filepath.Join(root, ".atrakta/contract.json")); err != nil {
		t.Fatalf("missing contract.json: %v", err)
	}
	if _, err := os.Stat(filepath.Join(root, ".atrakta/audit/events/install-events.jsonl")); err != nil {
		t.Fatalf("missing audit log: %v", err)
	}
	if _, err := os.Stat(filepath.Join(root, ".atrakta/audit/events/run-events.jsonl")); err != nil {
